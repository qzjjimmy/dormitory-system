const MENUS = [
  { key: 'dashboard', label: '服务台', icon: '◉', roles: ['student'], section: '学生服务', desc: '学生首页工作台', requiresDorm: true },
  { key: 'myDorm', label: '我的宿舍', icon: '⌂', roles: ['student'], section: '学生服务', desc: '宿舍信息与室友', requiresDorm: true },
  { key: 'repair', label: '报修申请', icon: '⚙', roles: ['student', 'admin', 'dormkeeper'], section: '学生服务', desc: '提交维修申请', requiresDorm: true },
  { key: 'fee', label: '费用查询', icon: '▣', roles: ['student', 'admin'], section: '学生服务', desc: '水电费账单与缴费', requiresDorm: true },
  { key: 'announcement', label: '校园公告', icon: '▤', roles: ['student', 'admin', 'dormkeeper'], section: '学生服务', desc: '查看系统公告' },
  { key: 'visitor', label: '访客预约', icon: '♟', roles: ['student', 'dormkeeper'], section: '学生服务', desc: '提交访客来访申请', requiresDorm: true },
  { key: 'rating', label: '服务评价', icon: '☆', roles: ['student'], section: '学生服务', desc: '评价宿舍服务与维修', requiresDorm: true },
  { key: 'transfer', label: '调宿申请', icon: '↔', roles: ['student', 'admin'], section: '学生服务', desc: '申请换宿舍/换床位', requiresDorm: true },
  ...
  { key: 'feedback', label: '意见反馈', icon: '✎', roles: ['student'], section: '学生服务', desc: '提交意见与建议', requiresDorm: true },
...
export function getVisibleMenus(role, hasDorm) {
  if (!role) return []
  return MENUS.filter(item => {
    if (!item.roles.includes(role)) return false
    if (role === 'student' && item.requiresDorm && !hasDorm) return false
    return true
  })
}
