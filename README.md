...
- **接口拦截**：`LoginInterceptor` 拦截未授权请求，排除 `/api/auth/login` 和 `/api/auth/register`，返回 401
- **参数校验**：所有 Controller 入口做空值校验，手机号正则验证
- **登出确认**：退出登录需二次确认
...
- 学生端：新生注册（特征采集+房间偏好）、我的宿舍、报修申请、费用查询、公告浏览、访客预约、服务评价、调宿申请（含推荐算法）、AI智能问答（按用户隔离）、智能通话（未分配仅见管理员/宿管）、意见反馈、个人中心（含密码修改）
- 管理员端：控制台、楼栋管理、智能宿舍分配（热力图+得分仪表盘+调宿推荐+一键审核）、入住管理（对接sys_user）、房间管理（自动聚合）、学生管理、宿管员管理、费用管理、公告管理、调宿审核、个人中心
- 宿管员端：工作台、入住管理、报修处理（一键审核）、访客登记、卫生检查、晚归登记、物品出入、消息通知、个人中心
